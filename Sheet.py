...
rt_sheet_id = "1bvoJSerq9--gjSZhjT6COgU_fzQ20tnYikrwz6KwYw0"
ct_sheet_id = "1bvoJSerq9--gjSZhjT6COgU_fzQ20tnYikrwz6KwYw0"
sheet_rts = gc.open_by_key(rt_sheet_id)
sheet_cts = gc.open_by_key(ct_sheet_id)
sheet_mmrs = {True:{True:sheet_rts.worksheet("Runner Leaderboard"), False:sheet_rts.worksheet("Bagger Leaderboard")},
              False:{True:sheet_cts.worksheet("Runner Leaderboard"), False:sheet_cts.worksheet("Bagger Leaderboard")}}
...
class Sheet(commands.Cog):
    ...
    async def google_sheets_mmr(self, member:discord.Member, is_rt=True, is_runner=True):
        
        name = member.display_name.lower().replace(" ", "")
        mmrs = sheet_mmrs[is_rt][is_runner]
    ...
    async def website_mmr(self, member:discord.Member, is_rt=True, is_runner=True):
        name = member.display_name.lower().replace(" ", "")
    ...
    async def mmr(self, member: discord.Member, is_rt=True, is_runner=True):
        if using_sheets:
            return await self.google_sheets_mmr(member, is_rt, is_runner)
        else:
            return await self.website_mmr(member, is_rt, is_runner)
